            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 pt-16 pb-10 animate-fade-up">
                ...
                <div class="flex flex-wrap items-end justify-between gap-4">
                    <div>
                        ...
                        <p class="text-gray-600 text-lg mt-3 max-w-2xl">Fresh crop and category deals from verified sellers near you.
                        </p>
                    </div>
                ...
                <!-- Tabs -->
                <div class="flex border-b border-gray-200 mb-8 mt-6">
                    <a href="{{ url('/deals?tab=crop') }}" class="mr-6 pb-4 text-base font-medium transition-colors relative {{ $tab === 'crop' ? 'text-green-600 border-b-2 border-green-600 font-semibold' : 'text-gray-500 hover:text-gray-700' }}">
                        Crop Deals
                    </a>
                    <a href="{{ url('/deals?tab=category') }}" class="pb-4 text-base font-medium transition-colors relative {{ $tab === 'category' ? 'text-green-600 border-b-2 border-green-600 font-semibold' : 'text-gray-500 hover:text-gray-700' }}">
                        Category Deals
                    </a>
                </div>
            </div>
            ...
            @if ($deals->count() > 0)
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    @foreach ($deals as $item)
                        @if($tab === 'category')
                            <a href="{{ url("/deals/$item->id?type=category") }}" class="reveal group block">
                                <div
                                    ...
                                    <div class="relative aspect-video bg-gray-100 overflow-hidden">
                                        ...
                                        <span
                                            ...
                                            <span class="bi bi-tag"></span> {{ $item->subcategory->category->name ?? 'Category' }}
                                        </span>
                                    </div>
                                    <div class="p-5 flex flex-col flex-1">
                                        <div class="flex items-center gap-2.5 mb-4">
                                            <img src="{{ $item->user->image ?? asset('default-avatar.png') }}"
                                                class="w-10 h-10 rounded-full object-cover border-2 border-green-100" alt=""
                                                loading="lazy">
                                            <div class="flex-1 min-w-0">
                                                <div class="font-semibold text-gray-900 truncate">{{ $item->user->name ?? 'N/A' }}
                                                </div>
                                        ...
                                        <div class="mb-3">
                                            <span class="text-sm text-gray-500">{{ $item->subcategory->name ?? 'N/A' }}</span>

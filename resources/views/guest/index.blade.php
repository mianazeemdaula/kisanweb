    ...
    <div class="bg-gradient-to-b from-white to-green-50 py-16">
        <div class="max-w-7xl mx-auto px-4">
            <div class="text-center mb-12">
                <h1 class="font-display text-6xl md:text-8xl font-extrabold mb-6 tracking-tight">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
                <div class="bg-white rounded-xl p-6 shadow-sm border border-green-100 hover-lift">
                    <div class="text-4xl font-bold text-green-600">{{ $todayRates->count() }}</div>
                    <div class="text-sm text-gray-600 mt-1 font-medium">Today's Rates</div>
                </div>
                <div class="bg-white rounded-xl p-6 shadow-sm border border-green-100 hover-lift">
                    <div class="text-4xl font-bold text-green-600">{{ $dealsCount }}</div>
                    <div class="text-sm text-gray-600 mt-1 font-medium">Active Deals</div>
                </div>
                <div class="bg-white rounded-xl p-6 shadow-sm border border-green-100 hover-lift">
                    <div class="text-4xl font-bold text-green-600">{{ $shopsCount }}</div>
                    <div class="text-sm text-gray-600 mt-1 font-medium">Verified Shops</div>
                </div>
                <div class="bg-white rounded-xl p-6 shadow-sm border border-green-100 hover-lift">
                    <div class="text-4xl font-bold text-green-600">{{ \App\Models\Crop::count() }}</div>
                    <div class="text-sm text-gray-600 mt-1 font-medium">Crop Types</div>
                </div>
            </div>
                        ...
                        @foreach ($todayRates as $rate)
                            <div
                                ...
                                <div class="flex justify-between items-start mb-3">
                                    ...
                                    <span
                                        class="text-xs bg-green-100 text-green-700 px-3 py-1 rounded-full font-medium">{{ $rate->city->name }}</span>
                                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach ($weeklyTrend->take(4) as $cropName => $rates)
                            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover-lift">
                                <h3 class="font-bold text-gray-900 mb-4 text-xl">{{ $cropName }}</h3>
                                <div class="space-y-2">
                                    @foreach ($rates->take(5) as $rate)
                                        <div class="flex justify-between items-center text-sm">
                                            <span
                                                class="text-gray-600 font-medium">{{ \Carbon\Carbon::parse($rate->rate_date)->format('M d') }}</span>
            ...
            <div class="text-center">
                <h3 class="font-display text-2xl font-bold text-gray-900 mb-6 tracking-tight">Connect With Us</h3>
                <div class="flex flex-wrap justify-center gap-4">
                    <a href="https://www.facebook.com/kisanstock"
                        class="px-6 py-3 bg-white border-2 border-green-600 text-green-700 font-semibold rounded-xl hover:bg-green-600 hover:text-white transition-all hover-lift">
                        Facebook
                    </a>
                    ...
                    <a href="https://play.google.com/store/apps/details?id=com.kisanstock.app"
                        class="px-8 py-3 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 transition-all shadow-lg hover-lift">
                        Download App
                    </a>
                </div>
            </div>

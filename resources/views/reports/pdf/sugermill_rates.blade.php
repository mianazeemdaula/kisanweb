    ...
    <main>
        <div class="w-1/2 h-auto mx-auto relative">
            <table class="w-full">
                <tbody>
                    <tr>
                        <td class="text-center text-2xl font-bold">شوگر ملز کے ریٹس</td>
                    </tr>
                    <tr>
            ...
            <table class="mt-2 mb-4 w-full">
                <thead class="bg-green-400">
                    <tr>
                        <th class="border">شوگر مل</th>
                        <th class="border">قیمت</th>
                        ...
                        <tr class="tr border-b text-xs border-gray-400">
                            @foreach ($rows as $rate)
                                <td class="text-center font-bold border border-gray-400 text-lg">
                                   {{ $rate->name_ur }}</td>
                                <td class='text-center border border-gray-400 text-lg w-24'>{{ $rate->rate->max_price }}</td>
                                <td class='text-center border border-gray-400 text-lg w-10'>
                                    @if ($rate->rate->min_price + $rate->rate->max_price > $rate->rate->min_price_last + $rate->rate->max_price_last)
                                        <i class="bi bi-arrow-up-short text-green-500"></i>
                                    @elseif ($rate->rate->min_price + $rate->rate->max_price < $rate->rate->min_price_last + $rate->rate->max_price_last)
                                        <i class="bi bi-arrow-down-short text-red-500"></i>
                                    @else
                                        {{ '-' }}
                                    @endif
                                </td>

...
@section('content')
    <div class="w-full max-w-4xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-bold text-slate-800">Edit Unit #{{ $unit->id }}</h1>
            <p class="text-sm text-slate-400 mt-1">Modify unit keys, names, or localized translations.</p>
        </div>
            ...
            <form action="{{ route('admin.units.update', $unit->id) }}" method="post">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" placeholder="e.g., kg, maund" name="key" value="{{ $unit->key }}" class="w-full">
                        @error('key')
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="e.g., Kilogram" name="name" value="{{ $unit->name }}" class="w-full">
                        @error('name')
                            <p class="text-red-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="e.g., کلو گرام" name="name_ur" value="{{ $unit->name_ur }}" class="w-full text-right" dir="rtl">
                        @error('name_ur')

...
@section('content')
    <div class="w-full max-w-4xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-bold text-slate-800">Edit Category #{{ $category->id }}</h1>
            <p class="text-sm text-slate-400 mt-1">Modify category information, parent associations, or icon tags.</p>
        </div>
            ...
            <form action="{{ route('admin.category.update', $category->id) }}" method="post">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="e.g., bi-flower3" name="icon" value="{{ $category->icon }}" class="w-full">
                        @error('icon')
                            <p class="text-red-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="e.g., Vegetables" name="name" value="{{ $category->name }}" class="w-full">
                        @error('name')
                            <p class="text-red-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="e.g., سبزیاں" name="name_ur" value="{{ $category->name_ur }}" class="w-full text-right" dir="rtl">
                        @error('name_ur')
